...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	# Convert TODOPatch and textureIm into floats
	TODOPatchFloat = TODOPatch*1.0
	textureImFloat = textureIm*1.0
	# Selected region from the texture: textureImFrame = (2*patchL+1, 2*patchL+1, 3)
	textureImRegion = np.ndarray(np.shape(TODOPatch))
	...
	for r in range(ssd_rows):
		for c in range(ssd_cols):
			...
			# Get the corresponding texture image region that has the same shape with TODOPatch
			textureImRegion = textureImFloat[r:r+patch_rows, c:c+patch_cols, :]

			# Calculate the squared difference
			# It has the same dimensionality with TODOMask
			squaredDiff = np.square(TODOPatchFloat - textureImRegion)

			# Squared sum
			squaredSum3Chan = 0

			# We don't count the pixels where TODOMask is not equal to 0.
			# We have to add the squared differences in SSD only if TODOMask = 0 at that pixel.
			for i in range(patch_rows):
				for j in range(patch_cols):
					if(TODOMask[i,j]==0):
						squaredSum3Chan += squaredDiff[i,j]
			
			squaredSum = np.sum(squaredSum3Chan)
			SSD[r,c] = squaredSum
	return SSD
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
	...
	for i in range(patchSize):
		for j in range(patchSize):
			...
			# Find the starting point of the texture frame
			textFrameRow = iMatchCenter - patchL + i
			textFrameCol = jMatchCenter - patchL + j

			# Find the starting point of the image patch
			imgPatchRow = iPatchCenter - patchL + i
			imgPatchCol = jPatchCenter - patchL + j

			# Copy the selected patch selectPatch into the image containing
			# the hole imHole for each pixel where TODOMask = 1.
			if(TODOMask[i,j] == 1):
				# Do the copying for all the color channels
				imHole[imgPatchRow, imgPatchCol, :] = textureIm[textFrameRow, textFrameCol, :]
	return imHole
